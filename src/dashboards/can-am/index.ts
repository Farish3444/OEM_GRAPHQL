...
export class CanAmDashboard implements ManufacturerInterface {
    ...
    try {
      ...
      for (let d = 0; d < arr.length; d++) {
        ...
        await this.page.select(
          "select#PlaceHolderMain_SelectProductLine",
          arr[d].productLine
        );
        ...
        // await this.page.waitForSelector('#formZone > div > table > tbody > tr > td > table > tbody > tr:nth-child(1) > th:nth-child(2)');
        ...
        let create_data = await this.page.evaluate(() => {
          ...
          let key_query = document
            .querySelectorAll(
              "#formZone > div > table > tbody > tr > td > table > tbody > tr"
            )[0]
            .querySelectorAll("th");
          ...
          let val_query = document
            ...
            .querySelectorAll("td");
          for (let i = 0; i < val_query.length; i++) {
            const val_element = val_query[i].innerText;
            val_array.push(
  ...
  private async reLogin(
    ...
    await this.page.type('input[name="j_id0:j_id5:Login1_UserName"]', username, 100);
    await this.page.type('input[name="j_id0:j_id5:Login1_Password"]', password, 100);
